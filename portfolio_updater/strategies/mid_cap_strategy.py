...
class MidCapStrategy(BaseStrategy):
    """
    Mid Cap Strategy — ASX XAO universe (top 300, excluding XJO top 200).

    Max positions : 20
    Sell (always) : current close ≤ 12-week rolling low             [Trend Stop]
    Sell (index below SMA only):
                    current close ≤ 12-week rolling low             [Trend Stop]
    ...
    Buy  (index above SMA, open slots exist):
                    ...
                    sized at equal weight: portfolio_equity ÷ 20.

    Symbol conversion: Trades sheet codes end in '.ASX'; stripped to base + '.au'
    before calling get_data (e.g. 'PME.ASX' → 'PME.au').
    """
    ...
    def check_for_sell_orders(
        ...
        Trend Stop    : close ≤ 12-week rolling low (fires regardless of index state).
